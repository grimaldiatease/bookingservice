...
import com.atease.booking.service.ReservationService;
...
/**
 * REST controller for managing {@link com.atease.booking.domain.Reservation}.
 */
@RestController
@RequestMapping("/api")
public class ReservationResource {
    ...
    public ResponseEntity<List<ReservationDTO>> getAllReservations(
        ...
        if ("payment-is-null".equals(filter)) {
            ...
            return new ResponseEntity<>(reservationService.findAllWherePaymentIsNull(), HttpStatus.OK);
        }

        if ("reservationdetails-is-null".equals(filter)) {
            log.debug("REST request to get all Reservations where reservationDetails is null");
            return new ResponseEntity<>(reservationService.findAllWhereReservationDetailsIsNull(), HttpStatus.OK);
        }

        if ("serviceinfo-is-null".equals(filter)) {
            log.debug("REST request to get all Reservations where serviceInfo is null");
            return new ResponseEntity<>(reservationService.findAllWhereServiceInfoIsNull(), HttpStatus.OK);
        }
